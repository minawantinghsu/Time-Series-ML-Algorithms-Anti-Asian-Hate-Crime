...
```{r}
ac <- ac %>% dplyr::select(date, `ASIAN / PACIFIC ISLANDER`, all)
bc <- bc %>% dplyr::select(date, `ASIAN / PACIFIC ISLANDER`, all)
```

#lm2  
```{r}
lm2 = lm(`ASIAN / PACIFIC ISLANDER` ~ all, data = bc)
summary(lm2)
```
...
```{r}
ac$ratio = round((ac$`ASIAN / PACIFIC ISLANDER`/ac$all),3)
bc$ratio = round((bc$`ASIAN / PACIFIC ISLANDER`/bc$all),3)
```
